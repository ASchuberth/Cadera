...
namespace txt {
	...
	void TextRender::setFontSize(float s) {
	...
	void TextRender::setCursorPos(glm::vec3 pos) {
	...
	std::vector<Vertex> TextRender::generateQuads() {
		...
		for (const auto& T : mTexts) {
			...
			setPlane(glm::normalize(T.second.cursorDirX), glm::normalize(T.second.cursorDirY));
			...
			for (const auto& c : T.second.text) {
				...
				xBear = T.second.textSize * Characters[c].x * mCursorDirX + T.second.offset.x * mCursorDirX;
				yBear = T.second.textSize * Characters[c].y * mCursorDirY + T.second.offset.y * mCursorDirY;
